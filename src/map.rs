        ...
        K: Copy + Eq,
{
    ...
    loop {
        ...
        let mut lock = LockThree::new(ti, fb, ob, tb);
        ...
        let invalid_path = {
            let bucket_fb = lock.bucket1(ti);
            let bucket_tb = lock.bucket3(ti);
            bucket_fb.key(fs) != &(*from).key ||
